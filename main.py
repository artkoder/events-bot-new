...
async def handle_dumpdb(message: types.Message, db: Database, bot: Bot):
    async with db.get_session() as session:
        ...
        vk_group_setting = await session.get(Setting, "vk_group_id")
    ...
    lines.append("")
    if vk_group_setting and vk_group_setting.value:
        lines.append(f"VK group: {vk_group_setting.value}")
    else:
        lines.append("VK group disabled")
    lines.append("")
    ...
    if catbox_setting and catbox_setting.value == "1":
        ...
        step += 1
    lines.append(
        f"{step}. Set VK group with /vkgroup {vk_group_setting.value if vk_group_setting and vk_group_setting.value else '<id>'}."
    )
