...
async def main():
    ...
    # Save JSON (do not write to repo root)
    project_root = Path(__file__).resolve().parents[2]
    output_dir = project_root / "artifacts" / "parser-output" / "dom-iskusstv"
    output_dir.mkdir(parents=True, exist_ok=True)

    output_file = output_dir / "dom_iskusstv_events.json"
    ...
    if results and results[0].get('selectors_used'):
        diag_file = output_dir / "dom_iskusstv_selectors.json"
