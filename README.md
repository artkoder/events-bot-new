...
- Массовые правки программы теперь собираются в YAML-файле по [шаблону v2](docs/festival_activities.schema.yaml): указываем `festival.slug`, даты проведения и список активностей.
- HOWTO в [`docs/festival_activities.md`](docs/festival_activities.md) объясняет, как скачивать шаблон, нормализовать площадки, заполнять блоки `daily_anytime` / `on_request` и отправлять файл через `/fest`.
