...
Карточка события в Telegram и VK включает блок «🌍 Туристам» со статусом (`Да`/`Нет`/`—`), счётчиком причин и отметкой о комментарии. Под сообщением остаётся клавиатура с пятью действиями:

- «Интересно туристам» / «Не интересно туристам» — сразу обновляют статус события (`tourist_label`, `tourist_label_by`, `tourist_label_at`, `tourist_label_source="operator"`), перерисовывают сообщение и пишут лог `tourist_label_update`.
- «Причины» — открывает меню с факторами `🎯 Специально для туристов`, `🧭 Есть только здесь`, `📍 Знаковое место`, `🏙️ Погружает в жизнь местных`, `🌿 Природа и виды`, `🍽️ Местная гастрономия`, `👨‍👩‍👧‍👦 Подходит для семьи`. Переключатели `➕/✅` обновляют `tourist_factors` и логируются как `tourist_factor_toggle`.
- «Готово» / «Пропустить» в меню причин возвращают базовую клавиатуру и отправляют сервисный тост.
- «✍️ Комментарий» — запускает окно ввода на 10 минут. Сообщение режется до 500 символов, сохраняется в `tourist_note` и фиксируется логом `tourist_note_saved`.
- «🧽 Очистить комментарий» — появляется при наличии заметки, стирает её и пишет `tourist_note_cleared`.

Меню причин хранит состояние в `tourist_reason_sessions` 15 минут; окно для заметки (`tourist_note_sessions`) закрывается через 10 минут и сбрасывает ожидание. Все действия доступны только неблокированным модераторам и администраторам, остальные пользователи получают «Not authorized».

Для аналитики и GPT-5 Deep Research предусмотрена команда `/tourist_export [--period | period=]`. Она формирует `.jsonl` со всеми полями события и колонками `tourist_*`, сортирует строки по дате и идентификатору, называет файл `tourist_export_<start>_<end>_<timestamp>.jsonl`, отвечает «No events found» при пустом наборе и логирует `tourist_export.request/start/done`.

Детали взаимодействия собраны в [user story](docs/tourist-label/user-story.md) и [исследовании](docs/tourist-label/research.md).
