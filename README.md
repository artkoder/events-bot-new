...
Кнопка «Причины» открывает меню с переключателями `➕/✅` для факторов `🎯 Нацелен на туристов`, `🧭 Уникально для региона`, `🎪 Крупный фестиваль или событие`, `🌊 Природа или знаковое место`, `📸 Фотогенично, понравится блогерам`, `🍲 Местный колорит и ремёсла`, `🚆 Простая логистика`; выбор сохраняется в `tourist_factors` по порядку справочника и логируется как `tourist_factor_toggle`. Меню живёт в `tourist_reason_sessions` 15 минут: по тайм-ауту или кнопкам «Готово»/«Пропустить» возвращается базовая клавиатура и показывается тост «Причины сохранены» или «Причины можно выбрать позже».
