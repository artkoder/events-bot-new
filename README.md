...
The bot generates promotional video clips with event highlights using Kaggle notebooks.

### Quick Start

1. Use `/v` to open the video announce menu
2. Select a profile and create a new session
3. Events with the 🎬 counter > 0 are automatically included
4. Review and edit the intro text generated by LLM
5. Choose a visual pattern (`STICKER`, `RISING`, or `COMPACT`)
6. Select a kernel and start rendering

### Intro Patterns

Three visual patterns are available for the intro screen:

| Pattern | Description |
|---------|-------------|
| **STICKER** | Alternating rotated text strips with fly-out animation |
| **RISING** | Composite lines with big numbers and 7° unidirectional shear |
| **COMPACT** | Tighter spacing with dynamic overlap |

The pattern preview is generated server-side by `video_announce/pattern_preview.py` without Kaggle.

### Assets

The Kaggle dataset includes:
- `payload.json` — event data with intro text, cities, date, pattern
- `BebasNeue-Bold.ttf` — font for Cyrillic text rendering
- `Pulsarium.mp3` — background audio

### Key Files

| Path | Purpose |
|------|---------|
| `video_announce/scenario.py` | Session management, pattern UI |
| `video_announce/selection.py` | Candidate ranking, payload generation |
| `video_announce/pattern_preview.py` | Server-side intro preview (PNG) |
| `video_announce/handlers.py` | Telegram callback routing |
| `kaggle/VideoAfisha/video_afisha.ipynb` | Kaggle kernel for video rendering |
