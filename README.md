...
Карточки событий в Telegram и VK показывают блок «🌍 Туристам» со статусом (`Да`/`Нет`/`—`), числом отмеченных причин и флагом о комментарии. Рядом остаётся клавиатура с кнопками:

- «Интересно туристам» и «Не интересно туристам» — мгновенно обновляют `tourist_label`, `tourist_label_by`, `tourist_label_at` (UTC) и `tourist_label_source="operator"`, перерисовывают текст и клавиатуру и пишут лог `tourist_label_update` с `event_id` и `user_id`.
- «Причины» — открывает меню с вариантами `🎯 Специально для туристов`, `🧭 Есть только здесь`, `📍 Знаковое место`, `🏙️ Погружает в жизнь местных`, `🌿 Природа и виды`, `🍽️ Местная гастрономия`, `👨‍👩‍👧‍👦 Подходит для семьи`. Кнопки переключаются между `➕` и `✅`, каждое действие пишет `tourist_factor_toggle`, состояние меню живёт 15 минут; выход через «Готово» или «Пропустить» просто возвращает основную клавиатуру.
- «✍️ Комментарий» — запускает ожидание сообщения на 10 минут. Ответ режется до 500 символов, сохраняется в `tourist_note`, обновляет служебные поля и лог `tourist_note_saved`. При наличии текста появляется «🧽 Очистить комментарий», которая удаляет заметку и фиксирует `tourist_note_cleared`.

Доступ к кнопкам и ручным выгрузкам есть только у неблокированных модераторов и администраторов; партнёры и гости получают «Not authorized».

Команда `/tourist_export [--period | period=]` возвращает `.jsonl` со всеми полями события, включая `tourist_*`, сортирует строки по дате и идентификатору, называет файл `tourist_export_<start>_<end>_<timestamp>.jsonl`, отвечает «No events found» при пустом наборе и логирует `tourist_export.request/start/done`. Подробности собраны в [user story](docs/tourist-label/user-story.md) и [исследовании](docs/tourist-label/research.md).
