...
Карточка события в Telegram и VK показывает блок «🌍 Туристам» со статусом (`Да`/`Нет`/`—`), счётчиком выбранных причин и пометкой о комментарии. Под ним размещается базовая клавиатура для ручной разметки:

- «Интересно туристам» и «Не интересно туристам» мгновенно переписывают `tourist_label`, `tourist_label_by`, `tourist_label_at` (UTC) и `tourist_label_source="operator"`, перерисовывают карточку без пересоздания сообщения и фиксируют `tourist_label_update`.
- «Причины» открывает меню с переключателями `➕/✅` для факторов `🎯 Нацелен на туристов`, `🧭 Уникально для региона`, `🎪 Крупный фестиваль или событие`, `🌊 Природа или знаковое место`, `📸 Фотогенично, понравится блогерам`, `🍲 Местный колорит и ремёсла`, `🚆 Простая логистика`. Выбор сохраняется в `tourist_factors` по порядку справочника и логируется как `tourist_factor_toggle`.
- «✍️ Комментарий» запускает сессию ожидания текста на 10 минут (`tourist_note_sessions`). Ответ обрезается до 500 символов, записывается в `tourist_note`, обновляет автора/дату/источник и создаёт лог `tourist_note_saved`.
- «🧽 Очистить комментарий» появляется, когда заметка уже есть, очищает `tourist_note` и пишет `tourist_note_cleared`.

В меню причин есть кнопки «Готово» и «Пропустить»: они закрывают список факторов, возвращают базовую клавиатуру и отправляют тост о сохранении или переносе выбора. Состояние меню живёт в `tourist_reason_sessions` 15 минут, после тайм-аута сбрасывается. Окно комментария истекает через 10 минут. Ручной контур доступен только неблокированным модераторам и администраторам; гости, партнёры и заблокированные пользователи получают «Not authorized».

Команда `/tourist_export [--period | period=]` доступна тем же ролям. Она формирует `.jsonl` с полями события и колонками `tourist_*`, сортирует строки по дате/ID, называет файл `tourist_export_<start>_<end>_<timestamp>.jsonl`, отвечает «No events found» при пустой выборке и пишет `tourist_export.request/start/done`.

Подробности по сценариям и исследованиям — в [user story](docs/tourist-label/user-story.md) и [исследовании](docs/tourist-label/research.md).
