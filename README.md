...
- «Причины» — открывает меню с вариантами `🏛️ История и культура`, `🏙️ Атмосфера города`, `🌿 Природа и активный отдых`, `🌊 Море и побережье`, `🍽️ Гастрономия`, `👨‍👩‍👧‍👦 Семейный отдых`, `🎉 События и фестивали`. Кнопки переключаются между `➕` и `✅`, каждое действие пишет `tourist_factor_toggle`, состояние меню живёт 15 минут; выход через «Готово» или «Пропустить» просто возвращает основную клавиатуру.
