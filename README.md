...
Каждая карточка события включает блок «🌍 Туристам», который показывает текущий статус (`tourist_label` → «Да»/«Нет»/«—»), количество выбранных причин и наличие комментария. Под блоком находится клавиатура с кнопками «Да»/«Нет», «🧩 Причины (N)» и «📝 Комментарий». Если уже есть текст, появляется дополнительная кнопка «♻️ Очистить».

Нажатия «Да»/«Нет» мгновенно сохраняют решение и обновляют карточку без перезагрузки. «🧩 Причины» открывает меню с чекбоксами для справочника (`history`, `sea`, `nature`, `food`, `family`, `events`). Состояние меню действует 15 минут; после таймаута оператору нужно заново нажать кнопку. «📝 Комментарий» стартует 10‑минутную сессию ожидания сообщения и сохраняет его в `tourist_note`; повторное нажатие позволяет переписать текст, а «♻️ Очистить» удаляет запись.

Текст комментария ограничен 500 символами — всё, что длиннее, автоматически обрезается при сохранении.

Доступ к блоку есть у всех неблокированных модераторов и администраторов (суперадмины включительно). Партнёры и незарегистрированные пользователи получают `Not authorized`. При каждом действии фиксируются `tourist_label_by`, `tourist_label_at` и `tourist_label_source`, что помогает отделять ручные правки в Telegram и VK.

Для аналитики предусмотрена команда `/tourist_export [period=...]`, которая формирует `.jsonl` с полями события и всеми колонками `tourist_*`. Диапазон принимает форматы `ГГГГ`, `ГГГГ-ММ` или `ГГГГ-ММ-ДД..ГГГГ-ММ-ДД`; пустая выборка возвращает «No events found». Командой могут пользоваться только неблокированные модераторы и администраторы (суперадмины включительно); партнёры получают «Not authorized». Подробные сценарии описаны в [user story](docs/tourist-label/user-story.md) и [исследовательских заметках](docs/tourist-label/research.md).
