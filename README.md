...
Run `/regdailychannels` to select Telegram channels for the daily posts and optionally set a VK group for cross‑posting. Use `/daily` to change posting times, send test posts or disable a channel. VK timings can also be adjusted with `/vktime today HH:MM` and `/vktime added HH:MM`.
...
with a list of connected channels, the current VK group (if any) and the steps
required to restore the dump on another server. After restoring, use `/vkgroup`
to configure the VK group again if needed. Send `/restore` with the dump file
attached to load it back.
