...
Цель: находить анонсы и апдейты по экскурсиям гидов в их личных Telegram-каналах, собирать по ним отдельный дайджест и публиковать его в Telegram без смешивания с обычными культурными событиями (`/daily`, month pages, weekend pages).
...
На уровне фичи источники стоит разделять минимум на четыре top-level archetypes:
...
- `GuideExcursionTemplate`
  - типовой маршрут
  - устойчивые факты и narrative
  - reference to past occurrences
  - reference to source materials / reports / reviews

### Логическая связь

`GuideProfile 1:N GuideExcursionTemplate 1:N GuideExcursionOccurrence`

Это лучше отражает реальность, чем плоская модель “один пост = одно событие”.
...
- это новый выход, апдейт существующего выхода или материал для шаблона;
- кто гид/гиды;
...
- clustering repeated excursions into templates;
