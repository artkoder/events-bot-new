# Парсинг событий из театров (/parse)

## Обзор

Команда `/parse` запускает парсинг событий из театров (Драмтеатр, Музтеатр, Кафедральный собор) через Kaggle-ноутбук.
...
Для парсинга событий с сайта pyramida.info используется отдельная функция в VK review flow.

См. [PYRAMIDA.md](./PYRAMIDA.md)
