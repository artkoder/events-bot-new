...
| `/recent_imports [hours]` | optional integer hours (default 24, max 168) | Superadmin only. Показывает события, которые были созданы или обновлены за окно `N` часов из `Telegram`, `VK` и `/parse`. В каждом разделе (`Telegram` / `VK` / `/parse`) список дедуплицирован по `event_id`; каждая строка начинается с `id`, затем показывает короткую иконку статуса (`✅` created, `🔄` updated), а название события ведёт на `telegraph_url` (или на `telegraph_path`, если URL ещё не сохранён явно). Если отчёт не помещается в одно сообщение Telegram, бот отправляет несколько страниц `Страница X/Y`. |
| `/popular_posts [N]` | optional integer limit (default 10) | Суперадмин: статистика “социальной популярности” постов, которые создали события (TG/VK). Показывает ТОП постов, где `views` или `likes` выше медианы внутри своего канала/сообщества: (1) окно 7 суток, где берётся последний доступный снапшот поста внутри `age_day=0..6`, (2) окно 3 суток, где берётся последний доступный снапшот поста внутри `age_day=0..2`, (3) окно 24 часа (`age_day=0`). В каждой строке: ссылка на исходник, список созданных событий (Telegraph + `id`), медианы канала и метрики поста, маркеры `⭐/👍`. Медиана считается по тому же `age_day` за `POST_POPULARITY_HORIZON_DAYS` (обычно 90 дней), а окно команды определяет только кандидатов в ТОП. В конце блока — диагностика: размер выборки (посты/источники; метрики vs импорт), счётчики `skip(...)`, и сколько постов выше медианы по `views/likes/оба` (после фильтров). |
