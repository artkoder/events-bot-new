...
| `/popular_posts [N]` | optional integer limit (default 10) | Суперадмин: статистика “социальной популярности” постов, которые создали события (TG/VK). Показывает ТОП постов, где `views` или `likes` выше медианы внутри своего канала/сообщества: (1) окно 3 суток (снапшоты `age_day=2`), (2) окно 24 часа (снапшоты `age_day=0`). В каждой строке: ссылка на исходник, список созданных событий (Telegraph + `id`), медианы канала и метрики поста, маркеры `⭐/👍`. Медиана считается по тому же `age_day` за `POST_POPULARITY_HORIZON_DAYS` (обычно 90 дней), а окно команды определяет только кандидатов в ТОП. В конце блока — диагностика: размер выборки (посты/источники; метрики vs импорт), счётчики `skip(...)`, и сколько постов выше медианы по `views/likes/оба` (после фильтров). |
