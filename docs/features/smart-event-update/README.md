...
Митигируемо:
- Для Telegram‑источников с `default_location` используется **проверка на конфликт**: если распознанная локация не совпадает с `default_location`, берём `default_location`, но раскладываем его в структурные поля (`location_name/location_address/city`) вместо копирования всей строки в `location_name`.
