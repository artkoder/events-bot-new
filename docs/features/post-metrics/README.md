...
- `POST_POPULARITY_MAX_AGE_DAY` (default `2`)
...
- Отбор событий по реакции аудитории (superadmin):
  - `/popular_posts [N]` — ТОП постов (TG/VK), где `views` и `likes` выше медиан внутри своего источника; выводит ссылку на исходник, созданные события (Telegraph + `id`) и цифры `median vs post`, плюс диагностическую строку по размеру выборки/скипам.
  - Для окна `3 суток` команда смотрит на посты, опубликованные за последние `~3` суток, и для каждого поста берёт самый поздний доступный снапшот `age_day<=POST_POPULARITY_MAX_AGE_DAY` (предпочитая `age_day=2`, но не требуя его жёстко). Это защищает отчёт от пустого результата в каналах, где за ночь не появилось новых сообщений и монитор не переснимал старые посты.
