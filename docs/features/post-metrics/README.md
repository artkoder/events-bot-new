...
- `1` — вторые 24 часа;
- `2` — третьи 24 часа;
- `3..6` — четвёртые-седьмые сутки (по умолчанию сохраняем до `6`, чтобы работал `ТОП за 7 суток`).
...
- для каждого `age_day` строго отдельно (`0` сравнивается только с историческими `0`, `2` только с историческими `2`).
...
- `POST_POPULARITY_MAX_AGE_DAY` (default `6`)
...
- `/popular_posts [N]` — ТОП постов (TG/VK), где `views` или `likes` выше медианы внутри своего источника; выводит ссылку на исходник, созданные события (Telegraph + `id`) и цифры `median vs post`, плюс диагностическую строку по размеру выборки/скипам.
  - кандидаты в ТОП берутся из окна отчёта (`24h` / `3d` / `7d`), но медиана для сравнения считается по `POST_POPULARITY_HORIZON_DAYS` и тому же `age_day`, а не по самому окну отчёта.
  - блок `3d` использует последний доступный снапшот поста внутри `age_day=0..2`, чтобы сильные посты не проваливались между `24h` и строгим `age_day=2`.
  - блок `7d` использует последний доступный снапшот поста внутри `age_day=0..6`, чтобы видеть устойчиво сильные посты за неделю без смешивания постов разных возрастов в одну медиану.
