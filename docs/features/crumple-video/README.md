...
- В `scheduling.py` теперь есть отдельный optional job для полностью автоматического `🧪 /v - Тест завтра`:
  - включается через `ENABLE_V_TEST_TOMORROW_SCHEDULED=1`;
  - по умолчанию запускается в `21:20 Europe/Kaliningrad` (`V_TEST_TOMORROW_TIME_LOCAL`, `V_TEST_TOMORROW_TZ`);
  - использует тот же `VideoAnnounceScenario.run_tomorrow_pipeline(...)`, что и ручной fast-path;
  - делает именно test-render на `12` сцен (`TOMORROW_TEST_MIN_POSTERS`);
