...
- Тестовый запуск:
  ...
  - `ярмарка` с fallback-датой закрытия (`end_date_is_inferred=1`) не считается “идущей до `end_date`” для `/v`: такой inferred-период не должен протаскивать одноразовую/weekly афишу из прошлой даты в завтрашнюю выборку.
  - для длинных событий (`ярмарка`/другие `date..end_date`) intro-диапазон считает не дату открытия, а ближайший релевантный день внутри окна подбора; если событие уже идёт к целевой дате, в intro берётся `target_date`, а не исторический старт.
- Боевой запуск:
  - 2-12 афиш;
  - все из базы анонсов;
...
## Надёжность Kaggle запуска

- После push notebook бот не начинает обычный polling сразу: сначала подтверждает через Kaggle metadata, что `crumple-video` действительно привязан к dataset текущей сессии (`video-afisha-session-<session_id>`).
- При terminal status (`COMPLETE`) бот ещё раз сверяет dataset-binding перед скачиванием output. Если kernel уже перепривязан к другому dataset чужим запуском, сессия переводится в `FAILED`, чтобы не публиковать чужой ролик и не привязывать stale output к неправильной сессии.
- В UI это убирает ложные ранние статусы сразу после push и делает явным отдельный этап подтверждения dataset в kernel.
...
- Размещение плашки обязано избегать уже занятого текста на афише: notebook сканирует весь постер по текстовой маске/edge-density, а не выбирает только из нескольких фиксированных нижних точек.
- Если в overlay нет названия события (например, нужно добавить только дату или локацию), первая строка не выделяется как крупный title-block, чтобы не раздувать плашку без необходимости.
