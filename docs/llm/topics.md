...
Классификатор может работать на разных LLM. По умолчанию используется **Gemma**
(`EVENT_TOPICS_LLM=gemma`, модель берётся из `EVENT_TOPICS_MODEL` или
`TG_MONITORING_TEXT_MODEL`). Для отладки можно переключить на 4o
