...
- Готовый ролик и логи отправляются в тестовый чат (`VIDEO_ANNOUNCE_TEST_CHAT_ID` или операторский), при наличии основного канала видео публикуется в `VIDEO_ANNOUNCE_MAIN_CHAT_ID` и записывается имя файла; встреченные события получают отметку `PUBLISHED_MAIN` и списывают одну единицу включения.
