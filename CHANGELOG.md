...
### Fixed
- **Deferred Rebuilds**: Устранена двойная пересборка страниц месяца (немедленная + отложенная). Теперь при наличии отложенной задачи немедленная пересборка пропускается, что значительно снижает нагрузку на API Telegraph.
- **Deferred Rebuilds**: Исправлен обход отложенности — `_drain_nav_tasks` больше не создаёт немедленные follow-up задачи если уже есть отложенная задача для event_id.
- **VK Inbox**: Исправлено отсутствие ссылки на Telegraph страницу в отчёте оператору ("✅ Telegraph — "). Теперь бот ожидает создания страницы перед отправкой ответа.
- **Deferred Rebuilds**: Убран синхронный вызов `refresh_month_nav` при обнаружении нового месяца.
- **Deferred Rebuilds**: `schedule_event_update_tasks` по умолчанию теперь использует `drain_nav=False`.
- **Deferred Rebuilds TTL**: Исправлено преждевременное истечение (expiration) отложенных задач.
- **Rebuild Notifications**: При автоматической пересборке страниц теперь суперадминам приходит уведомление.
- **Navigation Update**: При добавлении события на новый месяц обновляются футеры навигации на всех существующих страницах.
- **Year Suffix**: Исправлено отображение года в навигации.
- **Spam Removal**: Удалены отладочные сообщения `NAV_WATCHDOG`.
