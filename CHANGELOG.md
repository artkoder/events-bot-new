...
### Fixed
- **source_parsing**: Оптимизация производительности `/parse`:
  - Массовое добавление событий без промежуточного пересборки Telegraph-страниц
  - Отключена публикация в VK при парсинге источников
  - Единая пересборка `pages_rebuild` в конце обработки
  - Снижена задержка между событиями с 20с до 2с
  - Ожидаемое ускорение: с ~30 мин до ~5 мин
