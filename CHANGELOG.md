...
### Added
- **E2E Testing**: Добавлен фреймворк для E2E BDD тестов на Kaggle (`kaggle/E2ETests/`). Включает:
  - Оркестратор `execute_e2e.py` для автоматического запуска kernel и скачивания результатов
...
### Fixed
- **Events List Message Length**: Добавлен автоматический fallback на компактный формат (ID + Ссылка), если сообщение со списком событий превышает лимит Telegram (4096 символов).
- **Navigation Callback**: Исправлена ошибка `message is too long` при пагинации (`nav:`), также используется компактный формат.
...
- **Фестивали**: Команда `/festivals_fix_nav` теперь пропускает фестивали с завершившимися датами и без событий сегодня или в будущем. Это оптимизирует перестройку страниц и не трогает архивные фестивали.
