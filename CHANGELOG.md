...
- **Smart Update lollipop / Quality Retune**: strengthened the no-split downstream text-quality round by making `facts.prioritize.lead` explicitly format-first for opaque titles, broadening `editorial.layout` heading recovery for denser multi-theme cases via prompt/audit carries instead of hardcoded labels, and tightening `writer.final_4o` with structure-plan, format-clarity, length-band, and anti-reference-note prompt guidance.
...
- **Smart Update lollipop / Downstream Writer Tail**: baseline-comparison retune now pushes cross-promo and low-specificity filler suppression into `facts.prioritize`, adds an exhibition-history rescue for raw-fact mentions of the Great Depression / World War II, carries only `include` facts into downstream layout/pack coverage, marks partial literal lists via `literal_list_is_partial`, and finishes a new downstream `iter2` rerun with `editorial.layout` / `writer_pack.compose` at `events_with_flags=0` and `writer.final_4o` at `events_with_errors=0`, `events_with_warnings=0`.
- **Smart Update lollipop / Opaque Titles**: `facts.prioritize.lead` now applies title-opacity-aware fallback for bare `presentation` and `кинопоказ` titles, preferring event-action or film-defining facts over cast-only/project-description openings, while `editorial.layout` can preserve semantic headings for opaque-title presentation/screening cases outside `rich` density instead of collapsing them into headingless body blobs.
...
- Added `Smart Update lollipop` `v2.16.2 facts.extract` family-lab artifacts: a real `12`-event Gemma broad-run harness with resume + rate-limit wait handling, pre-merge per-stage traces, and a canonical review that splits the first family into `priority_core / secondary_broad / specialized_keep`.
- Added `Smart Update lollipop` `facts.extract` evidence-pack artifacts so every stage-run now has explicit `input.json` and the round has a consultation-ready `source -> prompt -> raw output -> result` packet plus a representative trace index.
- Added `Smart Update lollipop` `facts.extract` consultation round artifacts: a self-contained family brief, strict `Opus` review, `Gemini 3.1 Pro` counter-review, and a canonical synthesis that narrows the next shortlist to a small pruned bank plus baseline control.
- Added `Smart Update lollipop` `facts.extract` provenance artifacts so current `facts.extract_*.v1` ids now carry explicit `historical_prompt_id`, `historical_source_round`, `lollipop_slot`, and `lollipop_revision` across the stage bank, prompt inventory, consultation packet, per-stage inputs, and a canonical provenance-map report.
- Added `Smart Update lollipop` `facts.dedup` family artifacts: a new `baseline_diff -> cross_enrich -> audit` pilot harness on top of the pruned extraction shortlist, a real `4`-event Gemma run with full fact accounting, design/post-run `Opus` consultations, and a canonical synthesis that keeps the new architecture but blocks `facts.merge` until `baseline_diff` prompt precision is retuned.
- Added `Smart Update lollipop` `facts.dedup iter2` artifacts: a retuned `baseline_diff.v2` harness rerun on the full `12`-event casebook, new `iter2` lab metrics showing live `REFRAME` and healthier enrichment variance, plus narrow `Opus`/`Gemini` post-run reviews and a synthesis that keeps `facts.merge` blocked until cleaner-side `baseline_match` validation is relaxed or reworked.
- Added `Smart Update lollipop` `facts.dedup` prompt-fix consultation artifacts and `iter3` rerun artifacts: a self-contained brief with the exact `baseline_diff.v3(id-anchor)` contract, strict `Opus` plus single-launch `Gemini` review attempts, a full `12`-event dedup-only rerun over stored `facts.extract` outputs, and a new synthesis that upgrades `facts.dedup` to `GO` for the first `facts.merge` round with only small harness-only guardrails left.
- Added a canonical `docs/llm/smart-update-lollipop-funnel.md` route for the whole `lollipop` branch, documenting the family chain, current active statuses, cross-family salvage carries beyond `facts.extract`, and the immediate `facts.merge` plan so the branch no longer lives only in scattered reports.
- Added `Smart Update lollipop` `facts.merge iter1` artifacts: a first real `bucket -> resolve -> emit` family harness over stored `facts.extract` + `facts.dedup iter3` outputs on the full `12`-event casebook, a canonical evidence-pack with explicit `source/post -> prompt -> Gemma raw output -> parsed result`, strict `Opus` plus single-launch `Gemini 3.1 Pro` post-run consultations, and a synthesis that keeps `facts.prioritize` blocked until `resolve` collapse rules, tighter `bucket` guards, and a duplicate-suspect audit land in `facts.merge iter2`.
- Added a full `Smart Update lollipop` downstream prompt-pack consultation round (`Opus -> Gemini`) for all stages after `facts.merge`, plus a canonical synthesis that tightens the downstream spine, drops redundant stages (`facts.prioritize.infoblock_split`, `writer.spec`), and moves `writer_pack.compose` to a deterministic assembler.
- Added `Smart Update lollipop` `facts.merge iter2` artifacts: a prompt-level `bucket.v2 + resolve.v2` rerun on the same stored upstream data with explicit subsumption examples, screening/program/support guards, and a deterministic post-emit `merge_duplicate_suspect` audit; the round improves screening/exhibition bucketing but still leaves duplicate-heavy `program/people` cases unresolved, so `facts.prioritize` remains blocked.
- Added a narrow `Smart Update lollipop` `facts.merge.resolve iter2` consultation round: strict `Opus` identified only `2734` and `2731` as true resolve failures, classified `2673` and `2447` as audit false positives, supplied a full replacement `resolve.v3` prompt, and `Gemini 3.1 Pro` independently confirmed the same split plus the safety of the `SUBSUMES / SUBSUMED / DISTINCT` approach for the next rerun.
- Added `Smart Update lollipop` `facts.merge iter3` artifacts: a cached rerun over the same stored `facts.extract` + `facts.dedup iter3` outputs with a Gemma-friendly algorithmic `resolve.v3` prompt informed by prior Gemma prompt research; the round disproves the `resolve-only` hypothesis because `2734` and `2731` duplicates survive in separate clusters, adds a new overlap flag on `2759`, and shifts the next bottleneck from `resolve` to `bucket/linkage` plus overlapping baseline facts.
- Added a narrow `Smart Update lollipop` `facts.merge linkage` consultation round: single-launch `Gemini 3.1 Pro` reviewed the exact `bucket.v2` contract plus fail-cases `2734/2731/2759`, rejected `bucket.v3` overloading, recommended a dedicated `facts.merge.link.v1` stage, and supplied a full candidate linkage prompt for the next rerun.
- Added `Smart Update lollipop` `facts.merge iter4` artifacts: a cached rerun over the same stored upstream data with the new `facts.merge.link.v1` stage inserted between `bucket.v2` and `resolve.v3`; the round clears the real duplicate-heavy cases on `2734` and `2731`, reduces `events_with_flags` from `5` to `3`, and narrows the remaining blockers to `2759` overlap containment plus likely duplicate-audit false positives on `2673` and `2447`.
- Added `Smart Update lollipop` `facts.merge iter5` artifacts: a cached rerun over the same stored upstream data with `facts.merge.link.v2` plus bucket-aware `audit.v3`; the round clears `2759` and removes the prior false-positive flags on `2673` and `2447`, improving `events_with_flags` from `3` to `1` and leaving only one residual `2731` literal song-list linker inconsistency before `facts.prioritize`.
- Added a narrow `Smart Update lollipop` `facts.merge link song-list` consultation round: single-launch `Gemini 3.1 Pro` reviewed the exact residual `2731` `program_list` failure, recommended `Option C` (explicit schema contract plus narrow deterministic cleaner), and supplied the precise carry used for the next linker revision.
- Added `Smart Update lollipop` `facts.merge iter6` artifacts: a targeted regression-subset rerun on `2731 + 2673 + 2447 + 2759` with `facts.merge.link.v3` and a safe identical-song-list cleaner for `program_list`; the round clears the residual `2731` blocker and finishes the full subset with `events_with_flags = 0`.
- Added `Smart Update lollipop` `facts.prioritize iter2` artifacts: a first downstream family-lab over the full `12`-event `facts.merge iter5` casebook, `fact_id`-based Gemma `weight -> lead` stages with full `input -> prompt -> raw -> result` traces, a consultation-ready evidence-pack including source excerpts and raw facts, and an iter2 retune that enforces `logistics = high` plus a safe secondary lead fallback for screening/program edge-cases, finishing with `events_with_flags = 0`.
- Added `Smart Update lollipop` mixed-phase series post artifacts: a new canonical `lollipop` casebook document, a saved VK source packet for `vk.com/wall-179910542_11821`, and a full `Opus -> Gemini` consultation round that fixes a new `past-phase recap + future-phase anchor` source class and converges on a short `scope.extract.phase_map -> scope.select.target_phase -> facts.extract.phase_scoped` interceptor for future implementation.
- Added `Smart Update lollipop` mixed-phase prompt pack: canonical prompt texts for `scope.extract.phase_map.v1`, `scope.select.target_phase.v1`, and `facts.extract.phase_scoped.v1`, with the first two derived from the stronger `Opus` rewrite and the third assembled from the final `Opus + Gemini` synthesis.
- Added `Smart Update lollipop` mixed-phase interceptor dry-run harness and first lab run for `vk.com/wall-179910542_11821`, including trace artifacts in `source -> prompt -> raw output -> result` form and a safe sparse-future guard for missing `venue/time`.
- Added `Smart Update lollipop` editorial layout consultation artifacts and canonical prompt pack: a strict `Opus` round that writes the initial `editorial.layout.plan` contract, a `Gemini` review that moves density/list counting into deterministic precompute, and a new canonical `docs/llm/smart-update-lollipop-editorial-layout-prompts.md` documenting the final `precompute -> Gemma plan -> validate` design.
- Added `Smart Update lollipop` `editorial.layout iter1` artifacts: a full `12`-event family-lab over stored `facts.prioritize iter2` outputs, consultation-ready `source_excerpt -> prompt -> raw output -> result` traces, a deterministic cleaner/validator that closes the `2732` lead/program edge case without rerunning upstream, and a post-run `Gemini 3.1 Pro` review that upgrades the family to `GO` for deterministic `writer_pack.compose` while recommending prompt-level carries for `title_is_bare` and explicit `all_fact_ids`.
- Added `Smart Update lollipop` `writer_pack.compose iter1` artifacts: a deterministic `compose -> select(identity) -> audit` family-lab over the full `12`-event `editorial.layout iter1` casebook, a new canonical `docs/llm/smart-update-lollipop-writer-pack-prompts.md` contract, a consultation synthesis that combines `Opus` flat-section guidance with `Gemini` anti-double-exposure guidance via explicit `literal_items + coverage_plan`, and a post-run `Gemini` review that upgrades the family to `GO` for `writer.final_4o`.
- Added `Smart Update lollipop` `writer.final_4o iter1` artifacts: a strict `Opus -> Gemini 3.1 Pro Preview` design round that converges on a unified `{title, description_md}` contract with Python-side `keep` title override and no `_debug` self-report, a new canonical `docs/llm/smart-update-lollipop-writer-final-prompts.md` pack, a full `12`-event `gpt-4o` family-lab with deterministic infoblock/list validation, and a final `Gemini 3.1 Pro Preview` post-run `GO` after a narrow retune for list introductions and more cohesive prose.
### Added
