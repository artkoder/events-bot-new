...
def test_select_eventposter_render_urls_dedupes_by_ocr_title() -> None:
    ...
    rows = [
        (vk_url, None, "ОТ АВАНГАРДА ДО СОЦРЕАЛИЗМА", "text A", now - timedelta(days=1), "h1"),
        (catbox_url, None, "ОТ АВАНГАРДА ДО СОЦРЕАЛИЗМА", "text B", now, "h2"),
    ]
...
def test_select_eventposter_render_urls_dedupes_generic_titles_by_ocr_text() -> None:
    ...
    rows = [
        (vk_url, None, "Описание", ocr_text, now - timedelta(hours=2), "h1"),
        (catbox_url, None, "Описание", ocr_text, now, "h2"),
    ]
    urls, excluded = main._select_eventposter_render_urls(rows, prefer_supabase=False)
