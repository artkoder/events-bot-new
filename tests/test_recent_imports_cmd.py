...
async def test_recent_imports_load_rows_groups_created_vs_updated(tmp_path):
    ...
    try:
        ...
        tg_rows = await recent._load_recent_import_rows(
            db,
            source_key="telegram",
            start_utc=start,
            end_utc=now,
        )
        vk_rows = await recent._load_recent_import_rows(
            db,
            source_key="vk",
            start_utc=start,
            end_utc=now,
        )
        parse_rows = await recent._load_recent_import_rows(
            db,
            source_key="parse",
            start_utc=start,
            end_utc=now,
