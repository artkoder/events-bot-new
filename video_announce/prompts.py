...
def ranking_prompt() -> str:
    return (
        ...
        " Всегда включай отмеченные продвигаемые события, верни 12–15 позиций"
        " (максимум 15), зная что в интерфейсе по умолчанию выбирают 6–8."
        " Строго следуй инструкции оператора: не включай события, ей"
        " противоречащие, даже если подходящих мало. Укажи краткие русские"
        " причины. Не учитывай количество постеров как критерий. Ответ строго"
        " JSON со списком items без пояснений."
    )
...
def finalize_prompt() -> str:
    return (
        ...
        " постера дословно. Отдельно верни компактное поле about — одно"
